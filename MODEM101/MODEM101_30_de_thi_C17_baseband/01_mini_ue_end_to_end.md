# MÃ ĐỀ 01 — NR-SA R17 SMARTPHONE MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem UE 5G NR Standalone kiểu smartphone, lấy cảm hứng từ lớp chức năng công khai của Exynos Modem 5400/5410**. Trọng tâm biến thể: **FR1, numerology μ=1, cell search, PDSCH/PUSCH thu gọn, registration 5GS và PDU session**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — BASEBAND RUNTIME VÀ NR OFDM TRANSCEIVER
...
- Tự cài `struct bb_cpx16 { int16_t i, q; }`, ring SPSC dùng index nguyên tử, pool buffer generation-tagged và scheduler theo slot 0,5 ms.
- Tự cài radix-2 FFT/IFFT fixed-point, CP, resource grid, QPSK/16-QAM Gray, AWGN+CFO và AGC.
- Sinh và dò đúng ba PSS NR dài 127 theo TS 38.211; ước lượng timing/CFO, FFT, equalization một tap và CRC-24A.
- Phát uplink waveform từ transport block; tuyệt đối không được truyền bit trực tiếp giữa TX và RX.
...
## Câu 2 (30 điểm) — NR L2/L3 VÀ SECURITY CONTEXT
...
- Cài MAC multiplexing, BSR, 8 HARQ process với soft-combining LLR và timer.
- Cài RLC-UM 12-bit SN; PDCP 18-bit SN/HFN/COUNT, reorder window và anti-replay.
- Cài RRC `OFF→SEARCHING→CAMPED→CONNECTING→CONNECTED`; NAS `DEREGISTERED→REGISTERING→REGISTERED→SESSION_ACTIVE`.
- Integrity gửi NIA2/AES-CMAC job qua CRYPTO MMIO descriptor với COUNT/BEARER/DIRECTION; firmware chỉ giữ key-slot handle và nhận completion IRQ.
...
Khởi động, bắt cell PCI=42 tại CFO +1,8 kHz, nhận RRC Setup, hoàn tất security, registration, tạo PDU session, tải SDU 4096 byte; inject mất một codeword để buộc HARQ retransmission rồi link-loss và re-camp.
...
Điều kiện PASS của mã đề này: **đăng ký và thiết lập session thành công, SDU khôi phục byte-for-byte, có ít nhất một HARQ RETX, replay PDU bị loại và UE tự trở lại DATA sau link-loss**.
