# MÃ ĐỀ 20 — FAULT-CONTAINED COMMERCIAL BASEBAND MODEM
...
Sinh viên phải tự xây dựng từ đầu **nền tảng modem NR UE hoàn chỉnh có isolation, watchdog, crash recovery và secure observability**. Trọng tâm biến thể: **multi-domain runtime, MPU-like isolation, health supervision, bounded restart, state reconciliation và production telemetry**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — ISOLATED BASEBAND EXECUTION PLATFORM
...
- Chia domain L1, L2, control, IPC, key; memory object có owner/permission/generation và checked map/unmap.
- Cài scheduler priority+budget, mailbox quota, timer, watchdog heartbeat và supervisor dependency graph.
- Fault handler phân loại memory, deadline, queue, assert, peripheral; restart nhỏ nhất theo policy.
- Trace ring tamper-evident bằng hash chain, rate-limit, redact identity/key/payload và survive domain restart.
...
## Câu 2 (30 điểm) — RESILIENT NR MODEM STACK
...
- OFDM/QAM/CRC PHY, MAC HARQ, RLC, PDCP security, RRC/NAS registration/session trên virtual RF.
- State checkpoint chỉ chứa dữ liệu được phép; key domain cấp opaque handle và không restart chung với parser.
- Sau L2 restart, supervisor reconcile HARQ/RLC/PDCP generation hoặc reestablish; không giao stale PDU.
- AP↔CP IPC có reset epoch, request id, flow control và recovery indication.
...
Đăng ký và truyền liên tục 64 KiB; inject L1 deadline miss, RLC queue corruption, NAS parser exception, key-service heartbeat loss và AP flood theo lịch; mỗi fault cách nhau đủ để quan sát recovery.
...
Điều kiện PASS của mã đề này: **fault bị chứa đúng domain, không truy cập cross-domain, modem tự phục hồi hoặc clean-reconnect, payload sau recovery đúng và telemetry đủ root-cause mà không lộ bí mật**.
