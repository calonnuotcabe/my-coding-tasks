# MÃ ĐỀ 24 — NR SIDELINK PC5 V2X MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR sidelink Release 16/17 cho V2X, có direct PC5 unicast/groupcast và autonomous resource selection trên virtual RF**. Trọng tâm biến thể: **SLSS/PSBCH, PSCCH/PSSCH/PSFCH, SCI, Mode-2 sensing, sidelink HARQ và PC5 control**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — NR SIDELINK PHY VÀ RESOURCE POOL
...
- Cài sidelink synchronization signal/PSBCH, resource pool, PSCCH SCI stage-1, PSSCH SCI stage-2+TB, DM-RS và PSFCH feedback.
- TX/RX half-duplex và sensing window phải dùng cùng absolute slot clock; self-transmit slots không được tạo measurement giả.
- Cài QPSK/16-QAM, Polar control, LDPC data, rate matching, sidelink HARQ soft buffer và priority-aware processing deadline.
- Virtual channel hỗ trợ hai xe, hidden-node, Doppler, near-far và collision; receiver báo RSSI/RSRP/CRC theo resource.
...
## Câu 2 (30 điểm) — MODE-2 MAC, PC5-RRC VÀ SIDELINK SECURITY
...
- Cài sensing-based semi-persistent resource selection, exclusion threshold, reselection counter, reservation period và collision recovery.
- Cài unicast/groupcast/broadcast logical channels, sidelink HARQ ACK/NACK, RLC/PDCP duplication/reorder và QoS priority.
- PC5-RRC thiết lập/release link, capability và radio bearer; peer identity/context phải tách khỏi Uu RRC/NAS context.
- Security engine dùng group/unicast key-slot epoch, COUNT và anti-replay; member rời group không được giải mã epoch mới.
...
Ba UE V2X đồng bộ, hai UE chọn resource Mode 2 và gửi safety message 100 byte mỗi 20 ms; tạo hidden-node collision, PSFCH NACK, group rekey và peer restart giữa reservation period.
...
Điều kiện PASS của mã đề này: **SCI/TB decode đúng, resource reselection giải collision, safety message không giao trùng, stale reservation/key epoch bị loại và deadline được báo chính xác**.
