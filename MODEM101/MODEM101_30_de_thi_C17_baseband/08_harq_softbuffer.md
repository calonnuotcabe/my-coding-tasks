# MÃ ĐỀ 08 — LOW-LATENCY HARQ 5G MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE tập trung deadline, HARQ process và soft-buffer management như data path thương mại**. Trọng tâm biến thể: **slot pipeline, asynchronous HARQ, RV combining, ACK/NACK timing, bounded memory và URLLC priority**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — SLOT ENGINE VÀ HARQ PHY
...
- Cài slot clock, pipeline RX FFT→demap→decode và TX encode→map→IFFT với deadline event.
- 16 HARQ process chứa NDI, RV, TBS, codeword, deadline và soft buffer; validate transition IDLE/WAIT/RETX/DONE.
- Soft combine theo bit index sau rate recovery, saturating add; NDI mới phải reset đúng process.
- Virtual RF hỗ trợ burst erasure, AWGN và delayed feedback; late ACK không được áp vào generation mới.
...
## Câu 2 (30 điểm) — QoS MAC VÀ COMPLETE UE STACK
...
- MAC có SR/BSR, logical channel priority, grant, ACK/NACK/DTX và configured-grant thu gọn.
- RLC-UM cho URLLC, RLC-AM cho bulk; PDCP reorder/duplicate-discard và RRC/NAS session.
- Memory manager áp quota soft buffer theo UE/bearer; khi thiếu bộ nhớ phải hạ TBS hoặc từ chối grant deterministically.
- Trace mỗi deadline miss với stage latency nhưng không log payload hoặc key.
...
Hai flow: URLLC 64 byte mỗi 4 slot và bulk 32 KiB; inject erasure burst, ACK trễ, NDI toggle và memory pressure còn 25% soft-buffer.
...
Điều kiện PASS của mã đề này: **URLLC ưu tiên và không giao duplicate, bulk cuối cùng đúng byte, HARQ không trộn generation và modem không vượt memory budget**.
