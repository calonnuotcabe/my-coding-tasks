# MÃ ĐỀ 26 — FR2 PHASED-ARRAY BEAM MANAGEMENT MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR FR2 UE có phased-array model, initial beam acquisition, CSI-RS tracking, TCI/QCL và beam-failure recovery**. Trọng tâm biến thể: **SSB beam sweep, analog beam codebook, CSI-RS measurement, beam reporting, blockage và fast recovery**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — FR2 ARRAY/RF MODEL VÀ BEAMED PHY
...
- Model mảng 8×8 với codebook 64 beam, per-element phase/gain error, analog combiner, path angle, blockage và beam-dependent IQ stream.
- Cài SSB burst/beam sweep, PSS/SSS/PBCH acquisition theo beam, RSRP/L1 filtering và beam-indexed CFO/channel estimate.
- Cài CSI-RS resource/measurement, QCL assumptions, TCI state activation và guarded beam switch tại symbol boundary.
- Beam scan jobs đi qua RF/DMA timeline; không thể quan sát hai receive beam đồng thời với một RF chain.
...
## Câu 2 (30 điểm) — BEAM CONTROL, BFR VÀ COMPLETE FR2 STACK
...
- Cài beam manager states SEARCH/TRACK/SWITCH/BFD/BFR/RLF với N310-like evidence, candidate set, hysteresis và timers.
- Beam failure recovery tạo PRACH-like request trên candidate beam, chờ response, commit TCI hoặc fallback RRC reestablishment.
- MAC/HARQ/RLC/PDCP/RRC/NAS chạy session hoàn chỉnh; HARQ process phải giữ đúng beam/TCI generation của transmission.
- Thermal/EIRP budget giới hạn active elements, modulation và scan rate; policy không được starve control beam.
...
UE acquire qua SSB beam 7, registration và truyền 32 KiB ở FR2; path chuyển dần sang beam 9 rồi blockage đột ngột, BFD/BFR qua beam 21; inject stale CSI report và TCI activation trễ.
...
Điều kiện PASS của mã đề này: **beam acquisition/tracking đúng, stale report không đổi beam, BFR hoàn tất trước RLF hoặc reestablish sạch, payload đúng và EIRP/thermal budget được giữ**.
