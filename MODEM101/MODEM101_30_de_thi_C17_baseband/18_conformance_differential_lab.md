# MÃ ĐỀ 18 — CONFORMANCE-GRADE PROGRAMMABLE MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE hoàn chỉnh có reference path và optimized path để differential/conformance testing**. Trọng tâm biến thể: **bit-exact PHY, protocol invariants, golden traces, deterministic fault injection và explainable mismatch**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — REFERENCE VÀ OPTIMIZED MODEM CORES
...
- Tự viết hai implementation độc lập cho FFT/QAM/CRC và RLC/PDCP header handling: scalar reference và fixed-point optimized.
- Cả hai chạy cùng virtual RF/event trace nhưng không dùng chung logic cốt lõi ngoài type/schema.
- Cài canonical serialization để so symbol, LLR, TB, PDU, state và timer theo checkpoint.
- Mismatch phải báo layer, frame/slot, bearer/SN, field và first divergent byte; không chỉ báo pass/fail.
...
## Câu 2 (30 điểm) — COMPLETE UE CONTROL PLANE VÀ ORACLE
...
- RRC/NAS đưa cả hai core qua camp/connect/security/register/session; MAC HARQ và RLC/PDCP xử lý data.
- Oracle kiểm invariant độc lập: state legality, timer monotonicity, window, ownership, COUNT, integrity and delivery.
- Metamorphic tests: phase rotation+corr, padding, fragmentation, reorder within window và deterministic replay.
- Resource guard đặt giới hạn IQ, PDU, IE, queue và virtual time để malformed case không gây DoS.
...
Chạy golden scenario 8 KiB, 100 channel seeds và corpus malformed; giảng viên chèn lỗi endian CRC, fixed-point saturation và PDCP wrap vào một core.
...
Điều kiện PASS của mã đề này: **reference session hoạt động, differential runner định vị đúng ba divergence, không treo với malformed corpus và tạo report machine-readable**.
