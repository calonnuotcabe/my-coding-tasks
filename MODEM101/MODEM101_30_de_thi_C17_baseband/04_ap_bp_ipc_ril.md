...
Sinh viên phải tự xây dựng từ đầu **modem 5G split giữa Application Processor và Communication Processor với RIL/SIPC-like trust boundary**. Trọng tâm biến thể: **shared memory queues, doorbell, command schema, ownership, backpressure và isolation AP/baseband**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — CP RUNTIME, SHARED MEMORY VÀ IPC TRANSPORT
...
- Tạo hai circular queue AP→CP và CP→AP với descriptor, generation, length, flags, CRC32C và producer/consumer index.
- Cài acquire/release ordering, wraparound, credit flow-control và reset handshake; không dùng raw pointer qua trust boundary.
- Parser TLV hỗ trợ MODEM_POWER, SET_RADIO, GET_CELL, SETUP_DATA, TX_PACKET, RX_PACKET, RESET; unknown TLV phải bỏ an toàn.
- CP chạy task supervisor, L1, L2, RRC/NAS và IPC; queue đầy không được block task L1.
...
## Câu 2 (30 điểm) — NR MODEM STACK VÀ RIL SERVICE
...
- Cài virtual RF OFDM QPSK, sync, CRC và cell measurements.
- Cài MAC/RLC/PDCP, RRC connect và NAS registration/session tối thiểu.
- RIL daemon phía AP ánh xạ request-id→response, xử lý out-of-order, timeout, unsolicited indication và CP restart.
- Mọi length/count/offset từ AP phải checked arithmetic; quota theo client và redact trace chứa identity/key.
...
AP power-on CP, chọn NR-SA, đăng ký, tạo data call, gửi 1500 byte; đồng thời inject duplicate request-id, descriptor length vượt region, queue saturation và CP restart giữa transaction.
...
Điều kiện PASS của mã đề này: **gói hợp lệ đi xuyên AP→CP→virtual RF→CP→AP, input lỗi không corrupt SHM, RIL reconcile được restart và không trả nhầm response**.
