# MÃ ĐỀ 30 — RF CALIBRATION AND DIGITAL PREDISTORTION MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR FR1 UE hoàn chỉnh có factory/field RF calibration, transmitter digital predistortion và closed-loop power control**. Trọng tâm biến thể: **DC/LO leakage, IQ imbalance, PA nonlinearity, CFR/DPD, TSSI, temperature compensation, EVM và adjacent-channel leakage**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — RF IMPAIRMENT MODEL VÀ CALIBRATION ENGINE
...
- Virtual RF front-end phải model TX/RX DC offset, gain/phase IQ imbalance, LO leakage, frequency-dependent response, quantization và PA memory-polynomial nonlinearity.
- Cài factory calibration cho DC/IQ/LO, channel-response equalization và per-band gain table; estimator xuất coefficient+uncertainty, không chỉ corrected samples.
- Cài crest-factor reduction và memory-polynomial DPD adaptation từ observation receiver; coefficient update double-buffered tại slot boundary.
- TSSI/temperature sensor, power detector, ADC saturation và calibration DMA/IRQ phải đi qua MMIO; invalid sensor/loop divergence kích hoạt safe profile.
...
## Câu 2 (30 điểm) — CALIBRATED COMPLETE NR MODEM VÀ POWER CONTROL
...
- OFDM/DM-RS/QAM/LDPC PHY dùng calibration/DPD coefficients theo band, bandwidth, sample rate và temperature generation.
- Cài open/closed-loop uplink power control, ramp, MPR-like constraint, AGC và receiver IQ correction; không vượt configured conducted-power limit.
- MAC/RLC/PDCP/RRC/NAS registration/session chạy xuyên calibrated RF path; calibration task có priority thấp hơn L1 và bounded work per slot.
- NVM calibration store dùng two-slot journal, version, band id, temperature range, CRC/signature test và rollback; coefficient NaN/overflow bị reject.
...
Factory-calibrate band n78, register và truyền 64-QAM; sweep nhiệt độ -10→75°C làm PA drift, bật online DPD adaptation, inject observation-RX saturation và power-cut lúc lưu coefficient rồi reboot.
...
Điều kiện PASS của mã đề này: **EVM/ACLR surrogate và power nằm trong threshold đề cho, payload đúng, DPD divergence chuyển safe profile, NVM recover coefficient hợp lệ và L1 không miss deadline**.
