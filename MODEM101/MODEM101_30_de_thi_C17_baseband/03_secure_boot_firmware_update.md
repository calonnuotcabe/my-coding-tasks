# MÃ ĐỀ 03 — SECURE LTE BASEBAND MODEM WITH A/B FIRMWARE
...
Sinh viên phải tự xây dựng từ đầu **modem LTE Cat-4 hoàn chỉnh có secure boot, measured boot và cập nhật firmware A/B chống rollback**. Trọng tâm biến thể: **LTE OFDMA/SC-FDMA thu gọn, EPS attach, secure boot chain, image verification và recovery**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — BOOT ROM, IMAGE FORMAT VÀ BASEBAND RUNTIME
...
- Định nghĩa image `BBFW`: header, version, load address, segment table, SHA-256 và chữ ký Ed25519 theo RFC 8032 trên test key.
- Cài ROM loader kiểm tra checked-add, overlap, entry point, signature và monotonic rollback counter trước khi map executable.
- Cài A/B metadata có sequence, attempts, successful bit, CRC; update mất điện ở mọi byte vẫn chọn được một slot hợp lệ.
- Sau boot, khởi tạo task/queue/timer và vùng MPU riêng cho L1, protocol, IPC, key service.
...
## Câu 2 (30 điểm) — LTE UE DATA VÀ CONTROL PLANE
...
- Cài LTE grid 15 kHz, CP, QPSK/16-QAM, CRC, channel estimation pilot và equalizer.
- Cài MAC HARQ, RLC-UM, PDCP COUNT/replay; EPS RRC/NAS attach thu gọn.
- Key service cấp handle thay vì lộ key; zeroize context khi detach hoặc boot image khác.
- Firmware mới phải tự báo `BOOT_SUCCESS`; nếu timeout thì bootloader rollback.
...
Boot slot A v7, attach và truyền 1024 byte; cài slot B v8 qua các chunk reorder/duplicate, mô phỏng power-cut; thử image v6 và image segment overlap; cuối cùng boot v8, fail health-check một lần rồi recovery.
...
Điều kiện PASS của mã đề này: **không boot image giả mạo/rollback, A/B luôn recoverable, modem v8 attach và truyền đúng SDU sau lần recovery**.
