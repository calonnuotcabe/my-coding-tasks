# MÃ ĐỀ 28 — PRIVATE-5G URLLC/TSN INDUSTRIAL MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem private-5G UE làm TSN translator cho industrial control, có time synchronization, deterministic QoS và redundant transmission**. Trọng tâm biến thể: **gPTP-like clock, TSN schedule, configured grant, URLLC mini-slot, PDCP duplication và bounded jitter**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — SYNCHRONIZED URLLC PHY/MAC PIPELINE
...
- Cài hardware timestamp tại RF DMA ingress/egress, servo clock offset/drift và grandmaster change; monotonic time không được nhảy lùi.
- Cài μ=2 mini-slot pipeline, configured grant, repetition/dual-leg transmission, HARQ deadline và preallocated URLLC buffer pool.
- Time-aware gate list ánh xạ TSN cycle sang NR slots với guard band; late frame phải drop có reason thay vì gửi sang cycle sau.
- Jitter histogram, deadline miss và residence time được đo bằng virtual clock; trace không dùng wall-clock làm oracle.
...
## Câu 2 (30 điểm) — TSN TRANSLATOR, QoS FLOW VÀ RELIABILITY
...
- Cài Ethernet/TSN frame parser, stream identification, sequence recovery, duplicate elimination và per-stream policing.
- Ánh xạ TSN stream→5QI/QoS flow→logical channel→configured grant; config được shadow-build và commit ở cycle boundary.
- PDCP duplication trên hai legs, RLC-UM, RRC/NAS private-network registration và PDU session; leg failure không giao duplicate lên TSN.
- Clock uncertainty vượt threshold phải đóng scheduled gate an toàn; failover grandmaster và radio link có bounded recovery.
...
Hai control stream chu kỳ 1 ms và 4 ms chạy cùng bulk traffic; inject clock drift 40 ppm, grandmaster failover, một radio leg outage, duplicate packet và reconfiguration giữa cycle.
...
Điều kiện PASS của mã đề này: **control frames đúng thứ tự/cycle, duplicate bị loại, deadline/jitter trong budget khi còn đủ redundancy, unsafe clock đóng gate và bulk không starve control**.
