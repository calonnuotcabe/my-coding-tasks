# MÃ ĐỀ 29 — RELEASE-17 5G MBS MULTICAST MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE Release 17 nhận multicast/broadcast song song unicast session, có MRB lifecycle và PTM/PTP switching**. Trọng tâm biến thể: **5G MBS session, multicast radio bearer, PTM reception, service continuity, group security và congestion scaling**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — MBS-AWARE NR PHY/MAC RECEIVER
...
- Cài unicast và point-to-multipoint resource scheduling trên cùng carrier; PDCCH/PDSCH decode phải gắn RNTI/service generation đúng.
- FEC/HARQ policy phân biệt unicast và multicast profile; multicast repair/repetition không được dùng nhầm soft buffer của unicast.
- RF/PHY đo per-service BLER, discontinuous reception và service-area change; MBS traffic không được làm lỡ paging/control monitoring.
- Buffer fan-out dùng immutable handle/refcount bounded; một consumer chậm không giữ DMA buffer vô hạn.
...
## Câu 2 (30 điểm) — MBS SESSION, MRB VÀ GROUP SECURITY
...
- Cài service discovery/join/leave, MBS session id, service area, MRB setup/release và PTM↔PTP switching transaction.
- RLC/PDCP multicast reorder/duplicate handling tách biệt unicast COUNT; late join bắt đầu tại synchronization boundary.
- Group key-slot có epoch, validity interval và revocation; UE chưa join hoặc đã leave không được nhận plaintext epoch hiện tại.
- RRC/NAS/PDU session unicast vẫn hoạt động; cell reselection/handover reconcile MBS availability và báo service interruption.
...
UE register, mở unicast data và join hai MBS services; một service chuyển PTM→PTP do vùng phủ, group rekey, handover sang cell chỉ hỗ trợ một service, rồi leave; inject packet cũ và duplicate session command.
...
Điều kiện PASS của mã đề này: **MBS/unicast payload đúng và tách biệt, switch không giao trùng, revoked epoch bị loại, unavailable service báo đúng và không làm mất unicast session**.
