# MÃ ĐỀ 15 — LTE/NR DUAL-CONNECTIVITY MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem multi-RAT UE hỗ trợ LTE anchor, NR secondary node, split bearer và failover**. Trọng tâm biến thể: **EN-DC-like coordination, hai clock/PHY, RAT selection, bearer split và recovery**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — DUAL PHY VÀ COMMON BASEBAND RUNTIME
...
- Cài LTE PHY μ=0 và NR PHY μ=1 trên hai virtual RF port, cell search, OFDM/QAM/CRC riêng.
- Common clock chuyển timestamp giữa subframe LTE và slot NR; drift phải được theo dõi bounded.
- RTOS task/queue/buffer pool chia quota hai RAT; deadline NR không starve LTE control.
- Measurement database chuẩn hóa RSRP/RSRQ/SINR và freshness cho controller.
...
## Câu 2 (30 điểm) — MULTI-RAT CONTROL VÀ SPLIT BEARER
...
- Cài states OFF/LTE_CAMPED/LTE_CONNECTED/ENDC_ACTIVE/NR_FAILED/RECOVERING.
- LTE RRC/NAS làm anchor; NR secondary add/release; mỗi leg có MAC/HARQ/RLC, PDCP chung reorder/duplication.
- Policy engine chọn RAT theo signal, congestion, battery và hysteresis; config transaction có rollback.
- NR failure phải reroute outstanding PDCP PDU sang LTE mà không reuse COUNT hay giao trùng.
...
Attach LTE, add NR, truyền 32 KiB split 30/70; đưa NR vào blockage, LTE congestion, NR recovery và measurement stale; inject duplicate secondary-node response.
