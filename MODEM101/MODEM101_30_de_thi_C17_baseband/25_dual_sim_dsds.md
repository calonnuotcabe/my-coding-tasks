# MÃ ĐỀ 25 — DUAL-SIM DUAL-STANDBY BASEBAND MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem DSDS có hai subscription độc lập chia sẻ một RF chain, một DSP/FEC complex và AP↔CP interface**. Trọng tâm biến thể: **hai timing domain, paging collision, RF arbitration, subscription isolation, emergency preemption và context restore**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — SHARED-RF ARBITER VÀ DUAL TIMELINE
...
- Tạo hai modem context SIM-A/SIM-B với cell clock, RAT, band, RF calibration, paging occasions và DMA queues riêng nhưng dùng chung RF/FFT/FEC.
- Arbiter nhận reservation `{start,end,priority,retune_cost,subscription,generation}`; kiểm overlap, guard time và deterministic tie-break.
- Cài RF retune, PLL settle, AGC restore và calibration-cache theo band; sample trước settle phải đánh dấu invalid.
- ISR/completion phải route bằng owner+generation; completion từ SIM-A không được chạm buffer/key/state SIM-B.
...
## Câu 2 (30 điểm) — TWO COMPLETE STACKS VÀ DSDS POLICY
...
- Mỗi SIM có MAC/RLC/PDCP/RRC/NAS registration/session, key slots, COUNT, timers và AP request namespace riêng.
- Cài paging conflict policy, data-call suspension, missed-page accounting, emergency-call preemption và fair return-to-service.
- RRC/NAS context save/restore phải atomic; timer elapsed trong lúc RF thuộc SIM kia được xử lý theo policy, không replay callback cũ.
- AP IPC hỗ trợ subscription id, default-data switch và unsolicited indication; malformed id không được alias SIM-A.
...
SIM-A NR-SA có data session, SIM-B LTE idle; paging occasions trùng nhau, SIM-B nhận emergency page rồi chiếm RF, sau đó trả RF và SIM-A resume; inject stale DMA completion và AP default-data switch trùng epoch reset.
...
Điều kiện PASS của mã đề này: **hai subscription không rò state/key/payload, emergency được ưu tiên, SIM-A data tiếp tục đúng byte, paging loss đúng policy và không có stale completion**.
