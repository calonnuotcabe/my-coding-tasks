# MÃ ĐỀ 23 — LTE-M CAT-M1 eMTC MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem LTE Cat-M1/eMTC hoàn chỉnh cho tracker di động, hỗ trợ narrowband LTE, coverage enhancement, mobility và power saving**. Trọng tâm biến thể: **1,4 MHz/6 PRB, MPDCCH, LTE-M PDSCH/PUSCH, CE modes, handover, PSM và eDRX**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — CAT-M1 LTE PHY VÀ MPDCCH MONITORING
...
- Cài LTE-M 1,4 MHz sáu PRB với PSS/SSS/PBCH, CRS, MPDCCH search space, PDSCH và DFT-s-OFDM PUSCH/PUCCH.
- Cài CE Mode A/B repetition, frequency hopping, MPDCCH blind decode và soft combine; aggregation candidate phải bounded.
- Timing advance, uplink power control, oscillator drift và RF warm-up phải được điều phối qua slot/subframe timeline.
- DMA/FFT/FEC jobs có deadline, cookie/generation và fallback khi accelerator reset; không dùng full-band LTE shortcut.
...
## Câu 2 (30 điểm) — LTE-M MOBILITY, EPS BEARER VÀ POWER MANAGEMENT
...
- Cài LTE MAC HARQ, RLC-AM/UM, PDCP, RRC connection/reestablishment và EPS NAS attach/service request.
- Capability phải giới hạn Cat-M1 bandwidth/category/duplex/CE mode; network config vượt capability bị reject có cause.
- Measurement/reselection/handover hoạt động với repetition và measurement gap; PDCP COUNT không reset khi handover thành công.
- PSM/eDRX, paging occasion, periodic TAU và mobile-originated data phải dùng persistent timer journal chống stale wake.
...
Tracker attach ở CE Mode A, truyền GNSS record 4 KiB, chuyển sang CE Mode B khi suy hao tăng, handover sang cell kế cận rồi eDRX/PSM; inject MPDCCH false candidate và handover timeout.
...
Điều kiện PASS của mã đề này: **UE giữ EPS bearer qua mobility hoặc reestablish sạch, payload đúng, false grant không tới MAC và power-state/timer đúng qua simulated reboot**.
