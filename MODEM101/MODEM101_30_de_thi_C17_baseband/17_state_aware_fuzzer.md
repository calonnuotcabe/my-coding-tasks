# MÃ ĐỀ 17 — STATE-AWARE RESEARCH BASEBAND MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE có khả năng snapshot, deterministic replay và defensive state-aware robustness testing tích hợp**. Trọng tâm biến thể: **complete modem plus state checkpoint, grammar-aware input generation, invariant checking và automatic crash minimization; không viết exploit**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — INSTRUMENTED COMPLETE MODEM
...
- Cài OFDM/QAM/CRC PHY, MAC/HARQ, RLC, PDCP, RRC và NAS session chạy thật trên virtual RF.
- Mỗi task có stable state-id, coverage bitmap theo transition và deterministic virtual clock.
- Serializer snapshot toàn bộ buffer/queue/timer/HARQ/security/protocol state; cấm serialize raw pointer.
- Invariant gồm buffer ownership, window bounds, valid transitions, monotonic COUNT và no-deadlock progress.
...
## Câu 2 (30 điểm) — STATE-AWARE TEST DRIVER
...
- Tạo grammar cho RRC/NAS/MAC subset từ schema của chính modem, gồm valid và malformed bounded fields.
- Sequence generator chỉ mutate tại state hợp lệ, có dependency token và checkpoint selection theo coverage novelty.
- Watchdog phân loại reject/crash/hang/invariant; delta-debug sequence và payload để tạo regression case nhỏ nhất.
- Không được tạo exploit, shellcode, persistence hoặc tương tác thiết bị/mạng thật.
...
Driver phải tự đưa modem từ OFF đến SESSION_ACTIVE, checkpoint, thử 10.000 deterministic cases trên message/RF impairment, phát hiện ba bug được giảng viên cài trong toy modem rồi replay/minimize.
...
Điều kiện PASS của mã đề này: **modem truyền data bình thường, state driver đạt các milestone, mọi finding tái hiện 100%, không false-positive trên corpus sạch và output chỉ là test case phòng thủ**.
