# MÃ ĐỀ 11 — SECURE USER-PLANE 5G MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE có security plane tách biệt, key lifecycle, anti-replay và bearer isolation**. Trọng tâm biến thể: **PDCP COUNT/HFN, cipher/integrity API, reorder/duplication, security activation và zeroization**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — COMPLETE NR PHY/MAC/RLC FOUNDATION
...
- Cài OFDM QPSK/16-QAM, sync, channel estimate, CRC và AWGN/fading virtual RF.
- MAC HARQ và logical-channel multiplexing; RLC-UM segmentation/reassembly với 12-bit SN.
- RRC/NAS state đủ để tạo SRB và DRB; user-plane bị chặn trước SecurityModeComplete.
- Runtime phân vùng control/user/key service; IPC chỉ truyền key handle.
...
## Câu 2 (30 điểm) — PDCP SECURITY ENGINE
...
- Cài PDCP 18-bit SN, HFN và COUNT 32-bit; xử lý wrap không reuse keystream trong một key epoch.
- Cài crypto-accelerator model cung cấp NEA2 (AES-CTR) và NIA2 (AES-CMAC) với input COUNT/BEARER/DIRECTION; firmware gọi qua opaque key handle.
- RX window chống replay, reorder timer, duplicate discard và optional duplication trên hai legs.
- Security context có epoch, bearer, direction; activate atomically, zeroize khi release, từ chối stale epoch.
...
Đăng ký, security setup, truyền 10 KiB; inject replay, reorder quanh SN wrap, bit-flip tag, PDU đúng COUNT nhưng sai bearer, rồi rekey giữa hai SDU.
...
Điều kiện PASS của mã đề này: **plaintext hai đầu khớp, mọi forgery/replay/cross-bearer bị loại, COUNT không reuse và trace không lộ key/plaintext**.
