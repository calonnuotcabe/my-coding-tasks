# MÃ ĐỀ 19 — NR/NB-IoT NTN SATELLITE MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem direct-to-device NTN lấy cảm hứng từ hỗ trợ đồng thời NB-IoT NTN và NR NTN công khai của Exynos Modem 5400**. Trọng tâm biến thể: **large delay, Doppler, timing advance, ephemeris-like assistance, delayed HARQ và store-and-forward emergency data**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — NTN VIRTUAL RF VÀ SYNCHRONIZATION
...
- Channel tính propagation delay theo trace range, Doppler theo radial velocity, CFO drift, path loss và intermittent visibility.
- RX làm coarse/fine CFO, resampling timing, PSS correlation và tracking loop không giả định delay nhỏ.
- Cài NR OFDM và NB-IoT-like narrowband mode, QPSK, repetition combining, CRC và fixed resource grid.
- Virtual RF time-stamp mọi sample block; không được dịch packet trực tiếp để bỏ qua waveform.
...
## Câu 2 (30 điểm) — NTN MAC/RLC/RRC/NAS VÀ POWER
...
- HARQ timer/feedback có k-offset cấu hình lớn; mode không HARQ dùng repetition/ARQ ở RLC.
- Timing advance và uplink scheduling dùng assistance data có age/uncertainty; stale assistance phải resync.
- RRC/NAS NTN registration thu gọn, paging window và power state DEEP_SLEEP/ACQUIRE/CONNECTED.
- Store-and-forward queue có priority, TTL, dedup id, flash-journal và energy budget.
...
Thiết bị mất terrestrial coverage, acquisition vệ tinh LEO với Doppler +38 kHz và RTT biến thiên, đăng ký, gửi emergency message 512 byte; visibility mất giữa uplink rồi trở lại, assistance hết hạn.
...
Điều kiện PASS của mã đề này: **bắt/track link, message giao đúng một lần trước TTL, timer không fire sớm theo terrestrial assumption, queue sống qua simulated reboot và energy không vượt budget**.
