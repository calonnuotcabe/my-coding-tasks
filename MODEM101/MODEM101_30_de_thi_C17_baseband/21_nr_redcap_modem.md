# MÃ ĐỀ 21 — RELEASE-17 NR REDCAP INDUSTRIAL MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR Reduced Capability Release 17 cho wearable, camera công nghiệp và sensor gateway, giữ 5GS nhưng giảm bandwidth, RF chain và power**. Trọng tâm biến thể: **FR1 20 MHz, một RX/TX chain, half-duplex constraint, reduced capability signaling, eDRX và RRC Inactive**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — REDCAP RF/PHY VÀ HALF-DUPLEX SCHEDULER
...
- Cài FR1 μ=0/1 với active bandwidth tối đa 20 MHz, một antenna port, PSS/SSS/PBCH, PDCCH/PDSCH/PUSCH và DM-RS qua FFT/FEC accelerator.
- RF chain chỉ làm một việc RX hoặc TX tại một tick; arbiter phải phát hiện DL monitoring, PRACH, PUCCH/PUSCH và measurement-gap collision trước khi lập DMA.
- Cài reduced BWP switch, relaxed measurement cadence, narrow RF retune latency và warm-up/settling state trước khi sample hợp lệ.
- Tính energy theo RF_ON/RX/TX/DSP_WAKE/DDR_ACCESS; deadline PHY đúng nhưng không được giữ accelerator hoặc RF awake vô hạn.
...
## Câu 2 (30 điểm) — REDCAP CAPABILITY, RRC/NAS VÀ POWER STATE
...
- ASN.1 UPER capability phải khai báo RedCap, max bandwidth, antenna count, duplex mode, modulation và loại bỏ CA/dual-connectivity không hỗ trợ.
- Cài RRC IDLE/INACTIVE/CONNECTED, suspend/resume context, paging monitoring và small-data path; stale resume identity bị từ chối.
- Cài MAC HARQ/RLC/PDCP/NAS registration và PDU session; grant vượt capability phải reject thay vì truncate âm thầm.
- Power manager có ACTIVE, CDRX, EDRX_SLEEP, PSM, RF_WARMUP; timer và wake source dùng generation để loại event cũ.
...
UE RedCap cold-start trên cell 20 MHz, registration và session, ngủ eDRX, thức đúng paging occasion, gửi telemetry 8 KiB; inject DL grant trùng uplink half-duplex, BWP 40 MHz không hỗ trợ và stale resume context.
...
Điều kiện PASS của mã đề này: **UE chỉ nhận cấu hình đúng capability, không vi phạm half-duplex, resume/session thành công, telemetry đúng byte và năng lượng dưới budget của scenario**.
