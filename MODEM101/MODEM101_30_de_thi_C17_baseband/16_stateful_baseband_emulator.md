# MÃ ĐỀ 16 — FULL-SYSTEM SHANNON FIRMWARE MODEM EMULATOR
...
Sinh viên phải tự xây dựng từ đầu **trình mô phỏng baseband toàn hệ thống kiểu FirmWire nhưng chạy toy firmware do sinh viên tự viết và có quyền sử dụng**. Trọng tâm biến thể: **CPU/RTOS/peripheral emulation, firmware task graph, snapshot/restore và protocol-state fidelity**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — CPU, MEMORY VÀ PERIPHERAL EMULATOR
...
- Cài ISA 32-bit toy ARM-like gồm load/store, ALU, branch, call/return, privileged MMIO và exception.
- Page map có R/W/X, alignment, stack guard; deterministic instruction counter và interrupt delivery.
- Model timer, VIC, UART, RF DMA, shared-memory FIFO/SIPC, SIM và watchdog.
- Loader đọc firmware image do sinh viên tự tạo; validation segment/relocation/entry phải fail-closed.
...
## Câu 2 (30 điểm) — TOY BASEBAND FIRMWARE HOÀN CHỈNH
...
- Viết firmware task INIT/L1/MAC/RLC/PDCP/RRC/NAS/IPC chạy trên RTOS API của emulator.
- L1 xử lý virtual IQ OFDM QPSK; các lớp còn lại attach/register và truyền SDU end-to-end qua message queue.
- Cài APIs START/STOP/READ/WRITE/SNAPSHOT/RESTORE/LISTTASKS/INJECTMESSAGE và schema trace.
- Snapshot chứa CPU, RAM, MMIO, timers, queues, DMA, protocol state và generation; restore phải bit-deterministic.
...
Boot firmware, AP power-on qua SIPC, camp/register/session, truyền 2 KiB; snapshot tại CONNECTED, chạy thêm 500k instruction, restore rồi replay; inject peripheral-not-ready và stale DMA completion.
...
Điều kiện PASS của mã đề này: **firmware thực sự chạy qua CPU interpreter, hai replay cho trace hash giống nhau, state protocol/peripheral nhất quán và lỗi không thoát sandbox**.
