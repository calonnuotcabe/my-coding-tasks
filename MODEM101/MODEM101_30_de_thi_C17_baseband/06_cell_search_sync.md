# MÃ ĐỀ 06 — MOBILITY-FIRST 5G UE MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE tối ưu cell acquisition, measurement, reselection và handover trong môi trường nhiều cell**. Trọng tâm biến thể: **PSS/SSS, SS/PBCH block, measurement filtering, idle reselection và connected handover**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — CELL SEARCH VÀ TRACKING RECEIVER
...
- Sinh đúng 3 PSS và 336 SSS candidate theo PCI `3*N_ID1+N_ID2`; blind search timing/CFO trên stream IQ nhiều frame.
- Giải SS/PBCH block 240 subcarrier: PBCH DM-RS, descramble, rate recovery, Polar decode, CRC mask và MIB để dựng serving-cell clock.
- Tính RSRP/RSRQ/SINR, L1 average và IIR L3 filter; quản lý NCO để theo dõi residual CFO.
- Search phải bounded: giới hạn candidate, peak và work per frame; không cấp phát trong inner loop.
...
## Câu 2 (30 điểm) — RRC MOBILITY VÀ COMPLETE USER PLANE
...
- Cài neighbor database, S-criteria, hysteresis/Treselection, A3 event và time-to-trigger.
- Cài RRC idle/camped/connected/reestablishment; handover có prepare, execute, complete và rollback timer.
- Dữ liệu chạy qua OFDM→MAC HARQ→RLC-UM→PDCP; PDCP giữ COUNT qua handover và loại replay.
- Gap measurement phải phối hợp scheduler để không thu data carrier trong gap.
...
Ba cell PCI 10/20/30 có công suất thay đổi theo trace; UE cold-start ở CFO -3 kHz, camp PCI10, tạo session và truyền 6 KiB, reselection rồi A3 handover sang PCI20; inject handover command sai target.
...
Điều kiện PASS của mã đề này: **chọn cell đúng theo metric/timer, handover hợp lệ không duplicate SDU, command sai rollback về serving cell và session tiếp tục**.
