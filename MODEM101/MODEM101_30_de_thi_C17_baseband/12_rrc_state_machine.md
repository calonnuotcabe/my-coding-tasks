# MÃ ĐỀ 12 — CAPABILITY-RICH NR RRC MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE có RRC engine, capability negotiation và configuration transaction gần control plane thực tế**. Trọng tâm biến thể: **ASN.1-like bounded codec, cell group config, bearer lifecycle, measurement, reconfiguration và rollback**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — NR DATA PLANE VÀ RESOURCE MODEL
...
- OFDM/QAM/CRC/DMRS pipeline trên virtual RF, MAC HARQ, RLC và PDCP đầy đủ tối thiểu.
- Resource registry quản lý cell, BWP, logical channel, RLC entity, bearer và security context bằng generation handle.
- Mọi cấu hình mới được build ở shadow graph rồi commit atomically tại activation slot.
- Slot thread không parse control message và không allocation động.
...
## Câu 2 (30 điểm) — RRC CODEC VÀ STATE MACHINE
...
- Tự cài ASN.1 unaligned PER codec cho ASN.1 subset MIB, SIB1, Setup, SecurityMode, Reconfiguration, Release và Reestablishment được liệt kê trong đề; extension/constraint/optional bitmap phải đúng bit-level.
- Cài states IDLE/INACTIVE/CONNECTED cùng T300/T301/T310/T311 và transaction identifier.
- Capability gồm bands, SCS, BWP, CA combos, MIMO layers, modulation; reject unsupported config với cause chính xác.
- Reconfiguration failure phải rollback graph, timer và bearer; duplicate transaction trả idempotent response.
...
Camp qua MIB/SIB1, connect, activate security, add DRB và SCell, truyền 4 KiB; nhận reconfiguration có LCID trùng và BWP unsupported; sau đó radio-link failure và reestablishment.
...
Điều kiện PASS của mã đề này: **codec bounded, config tốt commit đúng slot, config lỗi không làm thay đổi active graph và data tiếp tục sau reestablishment**.
