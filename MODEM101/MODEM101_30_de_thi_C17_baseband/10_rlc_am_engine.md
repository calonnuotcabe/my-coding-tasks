# MÃ ĐỀ 10 — RELIABLE RLC-AM INDUSTRIAL 5G MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem private-5G UE cho telemetry công nghiệp, nhấn mạnh RLC-AM chính xác và bounded latency**. Trọng tâm biến thể: **segmentation/resegmentation, STATUS PDU, sliding window, poll/reassembly/status-prohibit timers và recovery**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — PHY/MAC TRANSPORT CHO PRIVATE 5G
...
- OFDM μ=1 QPSK/16-QAM, pilot equalization, CRC và virtual fading channel.
- MAC logical channels, configured grant, 8 HARQ process, timer và grant-size thay đổi.
- Runtime dùng fixed buffer pool, no allocation trong slot loop, queue watermark và watchdog.
- Cell access dùng RRC/NAS private-network identity thu gọn trước khi cho user data.
...
## Câu 2 (30 điểm) — RLC-AM ENTITY VÀ PDCP
...
- Cài 12-bit SN TX/RX window, SO, FI, LI, segmentation và resegmentation theo grant.
- Cài AMD/STATUS PDU với ACK_SN và NACK range; t-PollRetransmit, t-Reassembly, t-StatusProhibit.
- Duplicate/overlap segment không được corrupt SDU; checked arithmetic cho header/offset/length.
- PDCP COUNT/reorder và integrity bảo vệ command bearer; reset/reestablish không reuse COUNT.
...
Gửi 20 SDU kích thước 1..5000 byte qua grant biến thiên; reorder, duplicate, mất segment giữa, STATUS bị trì hoãn và wrap SN; sau đó RLC reestablishment.
...
Điều kiện PASS của mã đề này: **mọi SDU trước reestablishment giao đúng đúng một lần, timer/status đúng, không vượt window/buffer và session hoạt động lại sau reestablishment**.
