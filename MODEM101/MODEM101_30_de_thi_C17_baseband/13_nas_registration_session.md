# MÃ ĐỀ 13 — 5GS NAS AND SESSION MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR-SA UE hoàn chỉnh với registration, authentication, security mode và PDU session management**. Trọng tâm biến thể: **5GMM/5GSM state, timers/retry, identity privacy, authentication test vectors và user-plane activation**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — ACCESS STRATUM MODEM
...
- Cài NR virtual PHY OFDM/QPSK, cell search, CRC, MAC HARQ, RLC, PDCP và RRC connection.
- Cài SIM/USIM test service chứa SUPI giả, sequence, key test và monotonic auth state.
- NAS transport đi qua SRB, không được gọi trực tiếp giữa UE và network simulator.
- AP command interface có REGISTER, DEREGISTER, START_SESSION, STOP_SESSION, SEND_PACKET.
...
## Câu 2 (30 điểm) — 5GMM/5GSM NAS ENGINE
...
- Bounded TLV codec cho Registration, Authentication, Security Mode, Registration Accept/Reject và Service Request.
- Cài 5GMM substates, T3510/T3511/T3512-like timers, retry/backoff và reject causes.
- Cài PDU Session Establishment/Release, DNN, session id, QoS flow mapping và IP packet tunnel.
- Authentication/integrity dùng test primitive đã chỉ định; MAC sai, SQN stale, duplicate PTI và unknown IE phải fail-closed.
...
Cold registration bằng concealed test identity, authentication, NAS security, session internet; truyền IPv4 packet; inject auth MAC sai trước lần đúng, duplicate Accept, session reject rồi retry và periodic registration timer.
...
Điều kiện PASS của mã đề này: **5GMM/5GSM state/timer đúng, user-plane chỉ mở sau accept, malformed NAS không đổi state và packet cuối loopback đúng**.
