# MÃ ĐỀ 05 — MULTI-NUMEROLOGY NR FR1/FR2 MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem 5G NR dual-carrier có numerology linh hoạt và đường dữ liệu gần kiến trúc DSP/baseband thực tế**. Trọng tâm biến thể: **μ=0/1/2, bandwidth parts, FR1+FR2 virtual carriers, fixed-point DSP và deadline theo slot**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — FIXED-POINT PHY PIPELINE
...
- Cài Q1.15 complex arithmetic có saturation/rounding, block-floating FFT/IFFT và đo overflow.
- Tạo grid cho μ=0/1/2, CP đúng theo config, DM-RS thu gọn, QPSK/16/64-QAM và layer mapping tối đa 2 lớp.
- RX gồm AGC, PSS sync, CFO correction, pilot LS estimate, interpolation, MMSE 2×2 và soft demapper LLR.
- Mỗi carrier chạy worker riêng nhưng output phải deterministic theo slot/harq id.
...
## Câu 2 (30 điểm) — BWP, SCHEDULING VÀ PROTOCOL STACK
...
- Cài active/default BWP, guarded switch theo RRC command, slot-format và cross-numerology queue.
- MAC scheduler cấp PRB theo QoS; HARQ soft buffer có giới hạn; RLC/PDCP bảo toàn thứ tự SDU.
- RRC capability encode/decode khai báo numerology, MIMO layers, modulation và band-combination; reject cấu hình vượt capability.
- Thermal budget giả lập buộc hạ modulation/layer nhưng không phá timer control plane.
...
Camp FR1 μ=1, mở session, truyền 8 KiB; kích hoạt carrier FR2 μ=2, chuyển BWP, đưa CFO/phase-noise và fading 2×2; sau đó thermal throttle về một layer.
...
Điều kiện PASS của mã đề này: **hai carrier giữ đúng timeline, BLER/throughput được báo, payload đúng, BWP switch không mất control PDU và fixed-point không wraparound**.
