# MÃ ĐỀ 22 — TERRESTRIAL NB-IoT RELEASE-17 MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NB-IoT terrestrial hoàn chỉnh cho meter chạy pin, bám sát họ kênh narrowband và coverage-enhancement của LTE IoT**. Trọng tâm biến thể: **180 kHz carrier, NPSS/NSSS/NPBCH, NPRACH, NPDCCH/NPDSCH/NPUSCH, repetition và PSM/eDRX**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — NB-IoT NARROWBAND PHY VÀ COVERAGE ENHANCEMENT
...
- Cài carrier 180 kHz: NPSS/NSSS cell search, NPBCH/MIB-NB, NRS estimate, NPDCCH, NPDSCH và CRC/scrambling đúng bit order của profile đề.
- Uplink cài NPRACH preamble, NPUSCH multi-tone 15 kHz và single-tone 3,75/15 kHz; DFT/phase continuity phải đi qua IQ thật.
- Repetition engine kết hợp coherent/non-coherent theo channel estimate, có repetition count 1..2048, gap và accumulator saturation.
- Một HARQ process, RF retune/oscillator warm-up, frequency-error tracking và buffer budget phải hoạt động khi SNR xuống -13 dB.
...
## Câu 2 (30 điểm) — NB-IoT MAC/RLC/RRC/NAS VÀ LOW-POWER DATA
...
- Cài NPRACH access, NPDCCH scheduling, single-HARQ timing, MAC CE và backoff; stale grant sau repetition window phải bỏ.
- Cài RLC-AM, PDCP/NAS security context, EPS attach và non-IP/IP data bearer; control-plane data không được bypass integrity.
- RRC states và coverage level điều khiển repetition, paging/eDRX, PSM TAU và release assistance; NVRAM journal lưu timer/context có CRC+epoch.
- Quota flash queue, TTL, duplicate id và monotonic uplink counter phải bảo toàn qua simulated battery reset.
...
Meter bắt cell từ NPSS/NSSS ở -12,5 dB SNR, đọc MIB-NB, NPRACH/attach, gửi 512 byte qua 128 repetitions, vào PSM rồi wake bởi periodic TAU; inject duplicate NPDCCH và power-cut lúc commit counter.
...
Điều kiện PASS của mã đề này: **cell acquisition/attach đúng, payload giao đúng một lần, repetition không overflow, counter không rollback và modem trở lại PSM theo energy budget**.
