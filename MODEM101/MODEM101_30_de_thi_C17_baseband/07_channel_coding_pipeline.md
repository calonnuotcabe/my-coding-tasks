# MÃ ĐỀ 07 — NR CHANNEL-CODING ACCELERATOR MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE hoàn chỉnh có coding engine mô phỏng accelerator phần cứng và DMA descriptor**. Trọng tâm biến thể: **CRC, code-block segmentation, NR Polar/LDPC, rate matching, soft LLR và accelerator scheduling**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — CODING ENGINE VÀ DMA
...
- Cài đúng CRC-24A/B/C, transport-block/code-block segmentation và filler bits theo TS 38.212.
- Cài NR Polar encode/list-or-SC decode cho control subset K=32..140 và QC-LDPC layered min-sum cho BG2 với Zc={32,64,128}.
- Cài sub-block interleaving, circular-buffer rate match/recover cho RV=0..3; int8 LLR saturation và early parity termination.
- DMA ring có descriptor validation, scatter-gather, completion IRQ, timeout và reset accelerator.
...
## Câu 2 (30 điểm) — PHY/L2/L3 SỬ DỤNG ACCELERATOR
...
- OFDM QPSK/16-QAM ánh xạ coded bits qua virtual multipath channel rồi soft-demapper.
- MAC chọn MCS/TBS/RV; HARQ combine đúng vị trí rate-recovered; scheduler không reuse buffer trước completion.
- RLC/PDCP/RRC/NAS tạo một data session hoàn chỉnh và truyền SDU lớn qua nhiều transport block.
- Nếu accelerator timeout, fallback decoder phần mềm hoặc fail transaction có kiểm soát; không treo slot thread.
...
Registration qua control block NR Polar, truyền SDU 12 KiB bằng NR LDPC BG2; channel làm hỏng RV0 nhưng RV2 đủ giải; inject descriptor overlap và accelerator timeout ở block tiếp theo.
...
Điều kiện PASS của mã đề này: **control/data decode đúng, HARQ kết hợp RV thành công, descriptor lỗi bị từ chối và modem vẫn giữ registration khi accelerator phục hồi**.
