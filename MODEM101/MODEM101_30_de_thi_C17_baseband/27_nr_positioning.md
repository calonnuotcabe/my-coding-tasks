# MÃ ĐỀ 27 — NR POSITIONING-CAPABLE UE MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem NR UE hoàn chỉnh hỗ trợ DL-PRS/UL-SRS positioning, LPP control và đồng thời duy trì user-plane session**. Trọng tâm biến thể: **PRS correlation, RSTD/DL-TDOA, multi-RTT, SRS, clock bias/drift, measurement gap và location privacy**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — HIGH-RESOLUTION TIMESTAMP VÀ POSITIONING PHY
...
- Cài DL-PRS resource set, comb mapping, muting pattern, sequence generation và correlation TOA với fractional-sample interpolation.
- Model tối thiểu bốn TRP có clock offset, multipath/NLOS và known coordinates; RX báo RSTD, uncertainty, peak quality và resource id.
- Cài UL-SRS positioning waveform, timestamped DMA và multi-RTT exchange; RF/clock drift estimator phải tách oscillator bias khỏi propagation delay.
- Measurement gap và PRS scheduling không được phá PDCCH/PDSCH deadline; timestamp conversion phải overflow-safe.
...
## Câu 2 (30 điểm) — LPP, POSITION ENGINE VÀ COMPLETE UE STACK
...
- Cài ASN.1 UPER subset TS 37.355 cho capabilities, assistance data, measurement request/report, transaction id và abort/error.
- Position engine giải weighted least squares cho DL-TDOA/multi-RTT, loại outlier theo residual gate và xuất covariance/uncertainty.
- RRC measurement configuration, MAC gap, RLC/PDCP/NAS session và LPP transport phải đi qua protocol stack đúng bearer.
- Location request cần consent token/TTL/purpose; trace mặc định redact tọa độ và stale assistance không được dùng.
...
UE đăng ký và truyền data, nhận LPP request, đo bốn TRP với một NLOS outlier, phát SRS và tính vị trí; giữa measurement có handover và assistance-data update sai transaction id.
...
Điều kiện PASS của mã đề này: **vị trí nằm trong ngưỡng sai số đề cho, outlier bị hạ trọng số, data session không đứt, transaction sai bị loại và location không xuất khi consent hết hạn**.
