# MÃ ĐỀ 14 — LTE VoLTE/IMS VOICE MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem LTE UE hoàn chỉnh hỗ trợ EPS attach, IMS signaling thu gọn và real-time voice bearer**. Trọng tâm biến thể: **LTE PHY/L2, IMS registration/SIP subset, RTP jitter buffer, QoS bearer và call continuity**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — LTE ACCESS MODEM VÀ EPS BEARERS
...
- Cài LTE 15-kHz OFDMA downlink và DFT-s-OFDM uplink cho một antenna port, QPSK/16-QAM, CRS/DM-RS, CRC và PSS/SSS sync.
- MAC HARQ, RLC-AM cho signaling, RLC-UM cho voice, PDCP và EPS RRC/NAS attach.
- Bearer manager ánh xạ default bearer và dedicated voice bearer với priority/deadline.
- Virtual RF có fading burst và handover trace; media không được bypass protocol stack.
...
## Câu 2 (30 điểm) — IMS/SIP/RTP ENGINE
...
- Bounded parser/serializer cho REGISTER, INVITE, 100/180/200, ACK, BYE với Via/Call-ID/CSeq/Content-Length.
- Transaction timer và dialog state; reject header folding, duplicate critical header, length mismatch và oversized SDP.
- RTP sequence/timestamp, packetization PCM16, adaptive jitter buffer, loss concealment lặp frame và playout clock.
- Media/control chạy task khác nhau; signaling congestion không được block 20-ms audio deadline.
...
Attach LTE, IMS register, gọi outgoing, thiết lập voice bearer, truyền 5 giây PCM; inject reorder/loss/jitter, duplicate 200 OK và radio handover giữa cuộc gọi; cuối cùng BYE.
...
Điều kiện PASS của mã đề này: **dialog đúng, PCM output có đúng số frame theo chính sách concealment, không duplicate playout và cuộc gọi sống qua handover**.
