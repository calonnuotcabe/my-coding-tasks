# MÃ ĐỀ 02 — SHANNON-LIKE BASEBAND SoC MODEM
...
Sinh viên phải tự xây dựng từ đầu **modem LTE/NR thu gọn chạy trên RTOS và peripheral model kiểu kiến trúc quan sát được trong nghiên cứu FirmWire về Samsung Shannon**. Trọng tâm biến thể: **ARM-like task runtime, interrupt/timer, SHM/SIPC, RF/DSP/SIM peripheral và protocol task messaging**. Đây là bài thi engineering tích hợp, không phải ba bài rời: kết quả Câu 1 là nền chạy cho Câu 2, và Câu 3 chỉ đạt điểm khi toàn modem chạy end-to-end qua virtual RF.
...
## Câu 1 (30 điểm) — RTOS KERNEL VÀ PERIPHERAL BUS
...
- Cài priority scheduler tiền nhiệm xác định, TCB, mailbox, semaphore, timer wheel và IRQ controller.
- Cài memory map ROM/RAM/MMIO; mọi truy cập phải kiểm tra width, alignment và permission.
- Model `ShannonTimer`, `ShmFifo`, `SipcDoorbell`, `RfDma`, `SimCard` và watchdog; MMIO không hợp lệ tạo fault có kiểm soát.
- Boot tối thiểu các task `INIT`, `L1`, `MAC`, `RLC`, `RRC`, `NAS`, `SIPC`; task chỉ giao tiếp bằng message queue.
...
## Câu 2 (30 điểm) — FIRMWARE MODEM TASK GRAPH
...
- L1 nhận/phát IQ qua DMA descriptor; OFDM QPSK, CRC và AWGN loop phải chạy thật.
- MAC/RLC/PDCP xử lý PDU qua zero-copy buffer ownership; phát hiện double-free, stale handle và queue overflow.
- RRC/NAS thực hiện attach/registration thu gọn; SIM peripheral trả test identity và challenge response.
- SIPC phía AP có lệnh POWER, SET_RAT, GET_SIGNAL, START_DATA, SEND_SDU; parser length-prefixed phải fail-closed.
...
Boot firmware image mô tả bằng manifest, AP bật modem qua SIPC, SIM ready, camp cell, đăng ký, gửi 2048 byte; inject IRQ storm và một DMA descriptor sai rồi yêu cầu watchdog phục hồi protocol task mà không reboot toàn SoC.
...
Điều kiện PASS của mã đề này: **task graph boot đúng thứ tự, data loop thành công, lỗi DMA bị cô lập, IRQ storm bị rate-limit và AP vẫn nhận DATA_UP sau phục hồi**.
